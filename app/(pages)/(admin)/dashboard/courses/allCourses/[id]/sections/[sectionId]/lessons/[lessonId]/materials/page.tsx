...
export default function MaterialsPage({ params }: { params: Promise<{ id: string; sectionId: string }> }) {
  ...
  const [activeTab, setActiveTab] = useState<"materials" | "tasks" | "exams">("materials");
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="inline-flex items-center bg-white border border-gray-200 rounded-xl p-1 gap-1">
            <button
              ...
              className={`px-5 py-2 rounded-lg text-[14px] font-medium transition-colors ${activeTab === "materials" ? "bg-blue-600 text-white" : "text-gray-600 hover:bg-gray-50"}`}
            >
              Materiallar
            </button>
            <button
              onClick={() => setActiveTab("tasks")}
              className={`px-5 py-2 rounded-lg text-[14px] font-medium transition-colors ${activeTab === "tasks" ? "bg-blue-600 text-white" : "text-gray-600 hover:bg-gray-50"}`}
            >
              Vazifalar
            </button>
            <button
              onClick={() => setActiveTab("exams")}
              className={`px-5 py-2 rounded-lg text-[14px] font-medium transition-colors ${activeTab === "exams" ? "bg-blue-600 text-white" : "text-gray-600 hover:bg-gray-50"}`}
            >
              Imtihonlar
            </button>
          </div>
                      ...
                      {material.files.length > 0 ? (
                        <div className="flex flex-wrap items-center gap-2">
                          {material.files.map((file) => {
                            ...
                            return (
                              <span key={file.id} className="inline-flex items-center gap-2 pl-1.5 pr-3 py-1.5 bg-white border border-gray-200 rounded-lg text-[13px] font-medium text-gray-800">
