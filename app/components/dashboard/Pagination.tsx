  ...
  return (
    <div className="flex flex-col lg:flex-row lg:items-center justify-between py-4 gap-4 bg-[#F8F9FA] rounded-b-xl px-4">
      <div className="flex items-center gap-5 text-[14px] text-gray-700 font-semibold">
        <span>Sahifada {totalItems > 0 ? startIndex + 1 : 0}-{endIndex} gacha. Umumiy {totalItems}ta</span>
        
        <button onClick={onDownloadXLS} className="flex items-center gap-1.5 text-gray-700 hover:text-gray-900 transition-colors">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M8.5 11L10.5 15.5L8.5 20H11L12 17.5L13 20H15.5L13.5 15.5L15.5 11H13L12 13.5L11 11H8.5Z" fill="white"/>
          </svg>
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1.5 text-[14px] text-gray-700 font-medium relative group">
          ...
          <ChevronDown size={16} className="text-gray-500 absolute right-0 pointer-events-none" />
        </div>
        ...
        <div className="flex items-center gap-1 ml-4">
          {Array.from({ length: totalPages }).map((_, idx) => {
            ...
            if (page === 1 || page === totalPages || (page >= currentPage - 1 && page <= currentPage + 1)) {
              ...
              return <span key={page} className="text-gray-400 px-1 font-medium">...</span>;
            }
            return null;
          })}
